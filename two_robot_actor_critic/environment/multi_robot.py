...
class Robot:
    ...
    def plot_env(self):
        ...
        if self.current_target_frontier is not None:
            # 目標點 - 使用三角形標記
            triangle_color = '#800080' if self.is_primary else '#FFA500'
    ...
    def reset(self):
        ...
        if self.is_primary:
            # 主要机器人负责加载新地图
            self.li_map = (self.li_map + 1) % self.map_number  # 使用取模确保循环
            ...
            self.global_map = self.shared_env.global_map.copy()
            self.op_map = self.shared_env.op_map.copy()
    ...
    def initialize_visualization(self):
        ...
        # 導入必要的庫
        import matplotlib
        matplotlib.use('TkAgg')  # 明確設置後端為TkAgg
        import matplotlib.pyplot as plt
