...
def test_multiple_start_points_greedy(map_file_path, start_points_list, output_dir='results_greedy_targets_initial_diff'):
    """測試多個起始點位置，使用貪婪策略選擇目標
    ...
    for start_idx, (robot1_pos, robot2_pos) in enumerate(start_points_list):
        ...
        try:
            ...
            while not (robot1.check_done() or robot2.check_done()):
                ...
                # 貪婪選擇目標 - 為每個機器人選擇最近的 frontier
                valid_frontiers = len(frontiers)
                if valid_frontiers > 0:
                    # 計算每個 frontier 到機器人1的距離
                    robot1_distances = np.linalg.norm(frontiers - robot1.robot_position, axis=1)
                    # 選擇最近的 frontier 給機器人1
                    robot1_action = np.argmin(robot1_distances)
                    robot1_target = frontiers[robot1_action]
                    
                    # 計算每個 frontier 到機器人2的距離
                    robot2_distances = np.linalg.norm(frontiers - robot2.robot_position, axis=1)
                    
                    # 只在第一步（步驟為0）時避免兩個機器人選擇相同的目標
                    if steps == 0 and valid_frontiers > 1:
                        # 將機器人1選擇的 frontier 的距離設為無窮大，確保機器人2不會選擇它
                        robot2_distances[robot1_action] = float('inf')
                    
                    robot2_action = np.argmin(robot2_distances)
                    robot2_target = frontiers[robot2_action]
                    
                    # 打印機器人選擇的目標和距離
                    print(f"步驟 {steps}:")
                    print(f"  機器人1選擇frontier {robot1_action}，距離: {robot1_distances[robot1_action]:.2f}")
                    print(f"  機器人2選擇frontier {robot2_action}，距離: {robot2_distances[robot2_action]:.2f}")
            ...
            plt.title(f'Time-Coverage Analysis - Greedy Targets (Start Point {start_idx+1})', fontsize=16)
    ...
    try:
        ...
        for start_point in sorted(all_data.keys()):
            ...
            plt.xlabel('Time (steps)', fontsize=14)
            plt.ylabel('Coverage', fontsize=14)
            plt.title(f'Time-Coverage Analysis - Greedy Targets (Start Point {start_point})', fontsize=16)
            plt.grid(True, linestyle='--', alpha=0.7)
            plt.legend(fontsize=12)
            plt.ylim(0, 1.05)
        ...
        plt.title('Total Coverage Comparison - Greedy Targets', fontsize=16)
        plt.grid(True, linestyle='--', alpha=0.7)
        plt.legend(fontsize=12)
        plt.ylim(0, 1.05)
...
def main():
    # 指定地圖檔案路徑
    map_file_path = os.path.join(os.getcwd(), 'data', 'DungeonMaps', 'test', 'img_maze.png')
    ...
    # 設置輸出目錄
    output_dir = 'results_greedy_targets'
    
    # 運行貪婪測試
    test_multiple_start_points_greedy(map_file_path, start_points, output_dir)
