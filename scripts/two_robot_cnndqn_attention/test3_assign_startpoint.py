...
def test_model(model_path, map_file_path=None, robot1_pos=None, robot2_pos=None, num_episodes=5):
    """測試模型並儲存探索視覺化
...
def main():
    ...
    if not os.path.exists(specific_map_path):
        ...
        # 自定義起始位置 [x, y]
        # 這裡可以設置機器人的起始座標，格式為 [x, y]
        # 如果不想自定義，可以設為 None
        robot1_pos = [100, 100]  # 機器人1的起始位置
        robot2_pos = [150, 150]  # 機器人2的起始位置
    
    # 設置要運行的回合數量
    num_episodes = 3
    
    # 運行測試，使用指定的地圖和自定義起始位置
    test_model(model_path, map_file_path=specific_map_path, 
               robot1_pos=robot1_pos, robot2_pos=robot2_pos, 
