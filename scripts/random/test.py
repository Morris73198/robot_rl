...
def test_multiple_start_points(map_file_path, start_points_list, output_dir='results_random_targets'):
    """測試多個起始點位置，使用隨機目標選擇
        ...
        try:
            ...
            while not (robot1.check_done() or robot2.check_done()):
                ...
                if valid_frontiers > 0:
                    # 為每個機器人隨機選擇目標
                    robot1_action = random.randint(0, valid_frontiers - 1)
                    robot2_action = random.randint(0, valid_frontiers - 1)
                    
                    # 確保兩個機器人不選擇相同的目標（如果有多個frontier可供選擇）
                    if valid_frontiers > 1 and robot1_action == robot2_action:
                        # 隨機選擇一個不同的目標給機器人2
                        available_actions = list(range(valid_frontiers))
                        available_actions.remove(robot1_action)
                        robot2_action = random.choice(available_actions)
            ...
            plt.title(f'Time-Coverage Analysis - Random Targets (Start Point {start_idx+1})', fontsize=16)
    ...
    try:
        ...
        for start_point in sorted(all_data.keys()):
            ...
            plt.xlabel('Time (steps)', fontsize=14)
            plt.ylabel('Coverage', fontsize=14)
            plt.title(f'Time-Coverage Analysis - Random Targets (Start Point {start_point})', fontsize=16)
            plt.grid(True, linestyle='--', alpha=0.7)
            plt.legend(fontsize=12)
            plt.ylim(0, 1.05)
        ...
        plt.title('Total Coverage Comparison - Random Targets', fontsize=16)
        plt.grid(True, linestyle='--', alpha=0.7)
        plt.legend(fontsize=12)
        plt.ylim(0, 1.05)
