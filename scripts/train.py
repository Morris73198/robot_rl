...
def create_model_and_trainer(log_dir, robots):
    ...
    dummy_frontiers = np.zeros((batch_size, 2, 50, 2), dtype=np.float32)  # [batch, n_robots, n_frontiers, 2]
    dummy_robot_poses = np.zeros((batch_size, 2, 2), dtype=np.float32)    # [batch, n_robots, 2]
    dummy_robot_targets = np.zeros((batch_size, 2, 2), dtype=np.float32)  # [batch, n_robots, 2]
