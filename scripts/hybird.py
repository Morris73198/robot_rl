import os
# 禁用 GPU
os.environ["CUDA_VISIBLE_DEVICES"] = "-1"
import sys
import numpy as np
import matplotlib
# 設置 matplotlib 後端為 Agg (無需顯示)
matplotlib.use('Agg')
import matplotlib.pyplot as plt
from scipy import spatial
import csv
import random
import argparse

from two_robot_cnndqn_attention.environment.multi_robot_no_unknown import Robot
from two_robot_cnndqn_attention.models.multi_robot_network import MultiRobotNetworkModel
from two_robot_cnndqn_attention.config import MODEL_CONFIG, MODEL_DIR, ROBOT_CONFIG, REWARD_CONFIG

# 確保路徑在系統路徑中
current_dir = os.path.dirname(os.path.abspath(__file__))
parent_dir = os.path.dirname(current_dir)
sys.path.append(parent_dir)

# # 導入模型和配置
# try:
#     from two_robot_cnndqn_attention.models.multi_robot_network import MultiRobotNetworkModel
#     from two_robot_cnndqn_attention.config import MODEL_CONFIG, MODEL_DIR, ROBOT_CONFIG, REWARD_CONFIG
# except ImportError:
#     try:
#         from two_robot_dueling_dqn_attention.models.multi_robot_network import MultiRobotNetworkModel
#         from two_robot_dueling_dqn_attention.config import MODEL_CONFIG, MODEL_DIR, ROBOT_CONFIG, REWARD_CONFIG
#     except ImportError:
#         print("無法導入所需模塊，請確保項目路徑設置正確")
#         raise

# 混合策略決策函數
def hybrid_decision(robot1, robot2, model, frontiers, state, epsilon=0.3):
    """
    結合貪婪策略和深度學習模型的混合決策方法，並輸出詳細的分數計算過程
    
    Args:
        robot1, robot2: 機器人實例
        model: 訓練好的深度學習模型
        frontiers: 當前可用的frontier點
        state: 當前環境狀態
        epsilon: 控制探索程度的參數
        
    Returns:
        robot1_target, robot2_target: 選定的目標點
    """
    if len(frontiers) == 0:
        print("沒有可用的frontier點")
        return None, None
        
    valid_frontiers = min(model.max_frontiers, len(frontiers))
    print(f"可用frontier數量: {valid_frontiers}")
    
    # 獲取機器人位置和相關資訊
    robot1_pos = robot1.get_normalized_position()
    robot2_pos = robot2.get_normalized_position()
    map_dims = np.array([float(robot1.map_size[1]), float(robot1.map_size[0])])
    robot1_target_norm = (np.zeros(2) if robot1.current_target_frontier is None 
                    else robot1.current_target_frontier / map_dims)
    robot2_target_norm = (np.zeros(2) if robot2.current_target_frontier is None 
                    else robot2.current_target_frontier / map_dims)
    
    print(f"機器人1位置 (絕對座標): {robot1.robot_position}")
    print(f"機器人2位置 (絕對座標): {robot2.robot_position}")
    
    # 1. 貪婪策略 - 計算到各個frontier的距離
    robot1_distances = np.linalg.norm(frontiers - robot1.robot_position, axis=1)
    robot2_distances = np.linalg.norm(frontiers - robot2.robot_position, axis=1)
    
    # 距離的最大值和最小值 (用於後續正規化)
    r1_dist_max = max(robot1_distances) if max(robot1_distances) > 0 else 1.0
    r1_dist_min = min(robot1_distances)
    r2_dist_max = max(robot2_distances) if max(robot2_distances) > 0 else 1.0
    r2_dist_min = min(robot2_distances)
    
    print(f"\n距離統計:")
    print(f"機器人1 - 最近距離: {r1_dist_min:.2f}, 最遠距離: {r1_dist_max:.2f}")
    print(f"機器人2 - 最近距離: {r2_dist_min:.2f}, 最遠距離: {r2_dist_max:.2f}")
    
    # 排序距離，找出最近的frontier
    robot1_greedy_indices = np.argsort(robot1_distances)
    robot2_greedy_indices = np.argsort(robot2_distances)
    
    # 2. 模型預測 - 使用DRL模型獲取Q值
    frontiers_padded = np.expand_dims(model.pad_frontiers(frontiers), 0)
    predictions = model.predict(
        np.expand_dims(state, 0),
        frontiers_padded,
        np.expand_dims(robot1_pos, 0),
        np.expand_dims(robot2_pos, 0),
        np.expand_dims(robot1_target_norm, 0),
        np.expand_dims(robot2_target_norm, 0)
    )
    
    robot1_q_values = predictions['robot1'][0, :valid_frontiers]
    robot2_q_values = predictions['robot2'][0, :valid_frontiers]
    
    # Q值的最大值和最小值 (用於後續正規化)
    r1_q_max = max(robot1_q_values) if len(robot1_q_values) > 0 else 0
    r1_q_min = min(robot1_q_values) if len(robot1_q_values) > 0 else 0
    r2_q_max = max(robot2_q_values) if len(robot2_q_values) > 0 else 0
    r2_q_min = min(robot2_q_values) if len(robot2_q_values) > 0 else 0
    
    r1_q_range = r1_q_max - r1_q_min if len(robot1_q_values) > 0 else 1.0
    r2_q_range = r2_q_max - r2_q_min if len(robot2_q_values) > 0 else 1.0
    
    if r1_q_range == 0:
        r1_q_range = 1.0
    if r2_q_range == 0:
        r2_q_range = 1.0
    
    print(f"\nQ值統計:")
    print(f"機器人1 - 最小Q值: {r1_q_min:.4f}, 最大Q值: {r1_q_max:.4f}, 範圍: {r1_q_range:.4f}")
    print(f"機器人2 - 最小Q值: {r2_q_min:.4f}, 最大Q值: {r2_q_max:.4f}, 範圍: {r2_q_range:.4f}")
    
    # 排序Q值，找出模型認為最佳的frontier
    robot1_model_indices = np.argsort(-robot1_q_values)  # 從大到小排序
    robot2_model_indices = np.argsort(-robot2_q_values)
    
    # 3. 衝突檢測 - 計算每對可能目標的衝突風險
    conflict_matrix = np.zeros((valid_frontiers, valid_frontiers))
    for i in range(valid_frontiers):
        for j in range(valid_frontiers):
            if i == j:  # 如果兩個機器人選擇相同的frontier
                conflict_matrix[i, j] = 1.0
            else:
                # 計算兩個frontier之間的距離
                frontier_distance = np.linalg.norm(frontiers[i] - frontiers[j])
                # 如果frontier太近，增加衝突風險
                if frontier_distance < 30:  # 可調整的閾值
                    conflict_matrix[i, j] = 0.5  # 中等風險
    
    # 4. 決策融合 - 結合貪婪、模型和衝突評估
    # 為每個機器人創建候選目標列表
    robot1_candidates = []
    robot2_candidates = []
    
    # 加入貪婪選擇的前幾個目標
    for i in range(min(3, valid_frontiers)):
        if i < len(robot1_greedy_indices):
            robot1_candidates.append(robot1_greedy_indices[i])
        if i < len(robot2_greedy_indices):
            robot2_candidates.append(robot2_greedy_indices[i])
    
    # 加入模型選擇的前幾個目標
    for i in range(min(3, valid_frontiers)):
        if i < len(robot1_model_indices) and robot1_model_indices[i] not in robot1_candidates:
            robot1_candidates.append(robot1_model_indices[i])
        if i < len(robot2_model_indices) and robot2_model_indices[i] not in robot2_candidates:
            robot2_candidates.append(robot2_model_indices[i])
    
    print(f"\n機器人1候選目標索引: {robot1_candidates}")
    print(f"機器人2候選目標索引: {robot2_candidates}")
    
    # 計算每個機器人每個候選目標的綜合得分
    robot1_scores = {}
    robot2_scores = {}
    
    # 權重設置
    weight_greedy = 0.7   # 貪婪得分權重
    weight_model = 0.3    # 模型得分權重
    weight_diversity = 0.0  # 多樣性得分權重
    
    print("\n詳細得分計算：")
    print("-------------------------------------------------------------")
    print("機器人1 | 機器人2 | 貪婪得分1 | 貪婪得分2 | 模型得分1 | 模型得分2 | 多樣性得分 | 衝突風險 | 總分1 | 總分2")
    print("-------------------------------------------------------------")
    
    map_norm = np.linalg.norm(map_dims) if np.linalg.norm(map_dims) > 0 else 1.0
    
    for r1_idx in robot1_candidates:
        for r2_idx in robot2_candidates:
            # 檢查衝突風險
            conflict_risk = conflict_matrix[r1_idx, r2_idx]
            
            # 計算貪婪得分 (距離越近越好)
            greedy_score_r1 = 1.0 - (robot1_distances[r1_idx] / r1_dist_max)
            greedy_score_r2 = 1.0 - (robot2_distances[r2_idx] / r2_dist_max)
            
            # 計算模型得分 (Q值越高越好)
            model_score_r1 = (robot1_q_values[r1_idx] - r1_q_min) / r1_q_range
            model_score_r2 = (robot2_q_values[r2_idx] - r2_q_min) / r2_q_range
            
            # 計算多樣性得分 (鼓勵機器人選擇不同方向的frontier)
            diversity_r1r2 = np.linalg.norm(frontiers[r1_idx] - frontiers[r2_idx]) / map_norm
            
            # 綜合得分計算
            r1_total_score = (weight_greedy * greedy_score_r1 + 
                             weight_model * model_score_r1 + 
                             weight_diversity * diversity_r1r2 * (1 - conflict_risk))
                             
            r2_total_score = (weight_greedy * greedy_score_r2 + 
                             weight_model * model_score_r2 + 
                             weight_diversity * diversity_r1r2 * (1 - conflict_risk))
            
            # 輸出詳細得分計算過程
            print(f"{r1_idx:7d} | {r2_idx:7d} | {greedy_score_r1:10.4f} | {greedy_score_r2:10.4f} | "
                  f"{model_score_r1:9.4f} | {model_score_r2:9.4f} | {diversity_r1r2:11.4f} | "
                  f"{conflict_risk:9.2f} | {r1_total_score:5.4f} | {r2_total_score:5.4f}")
            
            # 更新得分
            if r1_idx not in robot1_scores or r1_total_score > robot1_scores[r1_idx]:
                robot1_scores[r1_idx] = r1_total_score
                
            if r2_idx not in robot2_scores or r2_total_score > robot2_scores[r2_idx]:
                robot2_scores[r2_idx] = r2_total_score
    
    print("-------------------------------------------------------------")
    
    # 選擇得分最高的目標
    if len(robot1_scores) > 0:
        r1_best_idx = max(robot1_scores.items(), key=lambda x: x[1])[0]
        print(f"機器人1最佳目標索引: {r1_best_idx}, 得分: {robot1_scores[r1_best_idx]:.4f}")
    else:
        r1_best_idx = robot1_greedy_indices[0] if len(robot1_greedy_indices) > 0 else 0
        print(f"機器人1沒有有效得分，使用貪婪選擇的最佳目標索引: {r1_best_idx}")
        
    if len(robot2_scores) > 0:
        r2_best_idx = max(robot2_scores.items(), key=lambda x: x[1])[0]
        print(f"機器人2最佳目標索引: {r2_best_idx}, 得分: {robot2_scores[r2_best_idx]:.4f}")
    else:
        r2_best_idx = robot2_greedy_indices[0] if len(robot2_greedy_indices) > 0 else 0
        print(f"機器人2沒有有效得分，使用貪婪選擇的最佳目標索引: {r2_best_idx}")
    
    # 最終檢查 - 避免極端情況
    if r1_best_idx == r2_best_idx and valid_frontiers > 1:
        # 如果兩個機器人選擇了同一個目標，讓其中一個重新選擇
        print(f"檢測到衝突：兩個機器人選擇了相同的目標 ({r1_best_idx})，正在重新選擇...")
        if np.random.random() < 0.5:
            # 找出機器人1第二好的選擇
            if r1_best_idx in robot1_scores:
                del robot1_scores[r1_best_idx]
            if len(robot1_scores) > 0:
                r1_best_idx = max(robot1_scores.items(), key=lambda x: x[1])[0]
                print(f"機器人1重新選擇的目標索引: {r1_best_idx}, 得分: {robot1_scores[r1_best_idx]:.4f}")
            else:
                # 退化為純貪婪選擇
                for idx in robot1_greedy_indices:
                    if idx != r2_best_idx:
                        r1_best_idx = idx
                        print(f"機器人1退化為貪婪選擇的目標索引: {r1_best_idx}")
                        break
    
    # 輸出最終選擇的目標信息
    print(f"\n最終選擇:")
    print(f"機器人1選擇的目標索引: {r1_best_idx}, 座標: {frontiers[r1_best_idx]}, 距離: {robot1_distances[r1_best_idx]:.2f}")
    print(f"機器人2選擇的目標索引: {r2_best_idx}, 座標: {frontiers[r2_best_idx]}, 距離: {robot2_distances[r2_best_idx]:.2f}")
    print(f"目標間距離: {np.linalg.norm(frontiers[r1_best_idx] - frontiers[r2_best_idx]):.2f}")
    
    # 返回選定的目標
    return frontiers[r1_best_idx], frontiers[r2_best_idx]
       
            

def save_plot(robot, step, output_path):
    """儲存單個機器人的繪圖
    
    參數:
        robot: 機器人實例
        step: 當前步驟數
        output_path: 儲存繪圖的路徑
    """
    plt.figure(figsize=(10, 10))
    robot.plot_env()
    plt.savefig(output_path, bbox_inches='tight')
    plt.close('all')  # 關閉所有圖形以釋放記憶體

def create_robots_with_custom_positions(map_file_path, robot1_pos=None, robot2_pos=None, train=False, plot=True):
    """創建使用特定地圖檔案和自定義起始位置的機器人
    
    參數:
        map_file_path: 指定地圖檔案的完整路徑
        robot1_pos: 機器人1的起始位置 [x, y]，如果為None則使用預設位置
        robot2_pos: 機器人2的起始位置 [x, y]，如果為None則使用預設位置
        train: 是否處於訓練模式
        plot: 是否繪製可視化
    
    返回:
        robot1, robot2: 兩個共享環境的機器人實例
    """
    # # 導入機器人類
    # try:
    #     from two_robot_cnndqn_attention.environment.multi_robot_with_unknown import Robot
    # except ImportError:
    #     try:
    #         from two_robot_dueling_dqn_attention.environment.multi_robot_with_unknown import Robot
    #     except ImportError:
    #         print("無法導入 Robot 類，請確保所需的模塊可用")
    #         raise

    # 創建一個自定義的 Robot 類繼承原始的 Robot 類
    class CustomRobot(Robot):
        @classmethod
        def create_shared_robots_with_custom_setup(cls, map_file_path, robot1_pos=None, robot2_pos=None, train=False, plot=True):
            """創建共享環境的機器人實例，使用指定的地圖檔案和起始位置"""
            print(f"使用指定地圖創建共享環境的機器人: {map_file_path}")
            if robot1_pos is not None:
                print(f"機器人1自定義起始位置: {robot1_pos}")
            if robot2_pos is not None:
                print(f"機器人2自定義起始位置: {robot2_pos}")
            
            # 創建第一個機器人，它會載入和初始化地圖
            robot1 = cls(0, train, plot, is_primary=True)
            
            # 載入地圖但不使用預設起始位置
            global_map, initial_positions = robot1.map_setup(map_file_path)
            robot1.global_map = global_map
            
            # 建立自由空間KD樹 (用於確保起始位置在可行區域)
            robot1.t = robot1.map_points(global_map)
            robot1.free_tree = spatial.KDTree(robot1.free_points(global_map).tolist())
            
            # 設置機器人位置
            if robot1_pos is not None and robot2_pos is not None:
                # 驗證位置是否有效 (在地圖範圍內且不是障礙物)
                robot1_pos = np.array(robot1_pos, dtype=np.int64)
                robot2_pos = np.array(robot2_pos, dtype=np.int64)
                
                # 確保位置在地圖範圍內
                map_height, map_width = global_map.shape
                robot1_pos[0] = np.clip(robot1_pos[0], 0, map_width-1)
                robot1_pos[1] = np.clip(robot1_pos[1], 0, map_height-1)
                robot2_pos[0] = np.clip(robot2_pos[0], 0, map_width-1)
                robot2_pos[1] = np.clip(robot2_pos[1], 0, map_height-1)
                
                # 確保位置不在障礙物上
                if global_map[robot1_pos[1], robot1_pos[0]] == 1:
                    print("警告: 機器人1的指定位置是障礙物，將移至最近的自由空間")
                    robot1_pos = robot1.nearest_free(robot1.free_tree, robot1_pos)
                    
                if global_map[robot2_pos[1], robot2_pos[0]] == 1:
                    print("警告: 機器人2的指定位置是障礙物，將移至最近的自由空間")
                    robot2_pos = robot1.nearest_free(robot1.free_tree, robot2_pos)
                
                # 使用自定義起始位置
                robot1.robot_position = robot1_pos
                robot1.other_robot_position = robot2_pos
            else:
                # 使用地圖中的預設位置或隨機選擇位置
                robot1.robot_position = initial_positions[0].astype(np.int64)
                robot1.other_robot_position = initial_positions[1].astype(np.int64)
            
            # 初始化其他屬性
            robot1.op_map = np.ones(global_map.shape) * 127
            robot1.map_size = np.shape(global_map)
            
            # 創建第二個機器人，共享第一個機器人的地圖和相關資源
            robot2 = cls(0, train, plot, is_primary=False, shared_env=robot1)
            
            robot1.other_robot = robot2
            
            # 重新初始化可視化
            if plot:
                # 清理舊的可視化
                if hasattr(robot1, 'fig'):
                    plt.close(robot1.fig)
                if hasattr(robot2, 'fig'):
                    plt.close(robot2.fig)
                
                # 初始化可視化
                robot1.initialize_visualization()
                robot2.initialize_visualization()
            
            return robot1, robot2
    
    # 使用自定義機器人類創建機器人
    return CustomRobot.create_shared_robots_with_custom_setup(
        map_file_path, 
        robot1_pos=robot1_pos, 
        robot2_pos=robot2_pos, 
        train=train, 
        plot=plot
    )

def test_hybrid_approach(map_file_path, start_points_list, model_path, output_dir='results_hybrid_approach'):
    """測試混合決策方法
    
    參數:
        map_file_path: 要使用的特定地圖檔案的路徑
        start_points_list: 包含多個起始點的列表，每個元素是 [robot1_pos, robot2_pos]
        model_path: 訓練好的模型路徑
        output_dir: 輸出目錄
    """
    # 確保輸出目錄存在
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    
    # 載入模型
    print(f"正在載入模型: {model_path}")
    model = MultiRobotNetworkModel(
        input_shape=MODEL_CONFIG['input_shape'],
        max_frontiers=MODEL_CONFIG['max_frontiers']
    )
    
    if os.path.exists(model_path):
        model.load(model_path)
        print("模型載入成功")
    else:
        print(f"警告: 在 {model_path} 找不到模型，無法進行混合決策測試")
        return
    
    # 導入地圖追蹤器
    try:
        from two_robot_cnndqn_attention.environment.robot_local_map_tracker import RobotIndividualMapTracker
    except ImportError:
        try:
            from two_robot_dueling_dqn_attention.environment.robot_local_map_tracker import RobotIndividualMapTracker
        except ImportError:
            print("無法導入 RobotIndividualMapTracker，請確保所需的模塊可用")
            raise
    
    # 創建CSV檔案來記錄每個步驟的覆蓋率數據
    csv_path = os.path.join(output_dir, 'coverage_data.csv')
    with open(csv_path, 'w', newline='') as csvfile:
        fieldnames = ['StartPoint', 'Step', 'Robot1Coverage', 'Robot2Coverage', 'IntersectionCoverage', 'UnionCoverage']
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
        writer.writeheader()
    
    # 為每個起始點運行測試
    for start_idx, (robot1_pos, robot2_pos) in enumerate(start_points_list):
        print(f"\n===== 測試起始點 {start_idx+1}/{len(start_points_list)} =====")
        print(f"機器人1起始位置: {robot1_pos}")
        print(f"機器人2起始位置: {robot2_pos}")
        
        # 創建當前起始點的輸出目錄
        current_output_dir = os.path.join(output_dir, f'start_point_{start_idx+1}')
        if not os.path.exists(current_output_dir):
            os.makedirs(current_output_dir)
        
        # 創建個人地圖目錄
        individual_maps_dir = os.path.join(current_output_dir, 'individual_maps')
        if not os.path.exists(individual_maps_dir):
            os.makedirs(individual_maps_dir)
        
        # 創建共享環境的機器人
        robot1, robot2 = create_robots_with_custom_positions(
            map_file_path,
            robot1_pos=robot1_pos,
            robot2_pos=robot2_pos,
            train=False,
            plot=True
        )
        
        # 創建地圖追蹤器
        tracker = RobotIndividualMapTracker(
            robot1, 
            robot2, 
            save_dir=individual_maps_dir
        )
        
        try:
            # 初始化環境
            state = robot1.begin()
            robot2.begin()
            
            # 開始追蹤個人地圖
            tracker.start_tracking()
            
            # 儲存初始狀態
            save_plot(robot1, 0, os.path.join(current_output_dir, 'robot1_step_0000.png'))
            save_plot(robot2, 0, os.path.join(current_output_dir, 'robot2_step_0000.png'))
            
            # 更新並儲存初始個人地圖
            tracker.update()
            tracker.save_current_maps(0)
            
            steps = 0
            intersection_data = []
            
            while not (robot1.check_done() or robot2.check_done()) :
                frontiers = robot1.get_frontiers()
                if len(frontiers) == 0:
                    break
                
                # 獲取當前位置和目標
                old_robot1_pos = robot1.robot_position.copy()
                old_robot2_pos = robot2.robot_position.copy()
                
                # 使用混合決策方法選擇目標
                robot1_target, robot2_target = hybrid_decision(robot1, robot2, model, frontiers, state)
                
                if robot1_target is None or robot2_target is None:
                    print("無法找到有效目標，退出迴圈")
                    break
                    
                # 打印選擇的目標
                robot1_distance = np.linalg.norm(robot1_target - robot1.robot_position)
                robot2_distance = np.linalg.norm(robot2_target - robot2.robot_position)
                print(f"步驟 {steps}:")
                print(f"  機器人1選擇目標: {robot1_target}，距離: {robot1_distance:.2f}")
                print(f"  機器人2選擇目標: {robot2_target}，距離: {robot2_distance:.2f}")
                
                # 移動機器人
                next_state1, r1, d1 = robot1.move_to_frontier(robot1_target)
                robot2.op_map = robot1.op_map.copy()
                
                next_state2, r2, d2 = robot2.move_to_frontier(robot2_target)
                robot1.op_map = robot2.op_map.copy()
                
                # 更新位置
                robot1.other_robot_position = robot2.robot_position.copy()
                robot2.other_robot_position = robot1.robot_position.copy()
                
                # 更新個人地圖追蹤器
                tracker.update()
                
                # 計算並記錄覆蓋率數據
                robot1_map = tracker.robot1_individual_map
                robot2_map = tracker.robot2_individual_map
                
                if robot1_map is not None and robot2_map is not None:
                    # 計算全局地圖的可探索區域總數
                    total_explorable = np.sum(robot1.global_map == 255)
                    
                    # 計算機器人1的覆蓋區域
                    robot1_explored = np.sum(robot1_map == 255)
                    robot1_coverage = robot1_explored / total_explorable if total_explorable > 0 else 0
                    
                    # 計算機器人2的覆蓋區域
                    robot2_explored = np.sum(robot2_map == 255)
                    robot2_coverage = robot2_explored / total_explorable if total_explorable > 0 else 0
                    
                    # 計算兩個機器人都探索過的區域（交集）
                    intersection = np.sum((robot1_map == 255) & (robot2_map == 255))
                    intersection_coverage = intersection / total_explorable if total_explorable > 0 else 0
                    
                    # 計算任一機器人探索過的區域（聯集）
                    union = np.sum((robot1_map == 255) | (robot2_map == 255))
                    union_coverage = union / total_explorable if total_explorable > 0 else 0
                    
                    # 記錄數據
                    intersection_data.append({
                        'step': steps,
                        'robot1_coverage': robot1_coverage,
                        'robot2_coverage': robot2_coverage,
                        'intersection_coverage': intersection_coverage,
                        'union_coverage': union_coverage
                    })
                    
                    # 寫入CSV
                    with open(csv_path, 'a', newline='') as csvfile:
                        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
                        writer.writerow({
                            'StartPoint': start_idx+1,
                            'Step': steps,
                            'Robot1Coverage': robot1_coverage,
                            'Robot2Coverage': robot2_coverage,
                            'IntersectionCoverage': intersection_coverage,
                            'UnionCoverage': union_coverage
                        })
                
                state = next_state1
                steps += 1
                
                # 每 10 步儲存繪圖和個人地圖
                if steps % 10 == 0:
                    save_plot(robot1, steps, os.path.join(current_output_dir, f'robot1_step_{steps:04d}.png'))
                    save_plot(robot2, steps, os.path.join(current_output_dir, f'robot2_step_{steps:04d}.png'))
                    
                    # 儲存當前個人地圖
                    tracker.save_current_maps(steps)
                    
                    # 輸出進度信息
                    if len(intersection_data) > 0:
                        print(f"步數: {steps}, 聯合覆蓋率: {intersection_data[-1]['union_coverage']:.1%}")
            
            # 儲存最終狀態
            save_plot(robot1, steps, os.path.join(current_output_dir, f'robot1_final_step_{steps:04d}.png'))
            save_plot(robot2, steps, os.path.join(current_output_dir, f'robot2_final_step_{steps:04d}.png'))
            
            # 儲存最終個人地圖
            tracker.save_current_maps(steps)
            
            # 生成每個起始點的覆蓋率時間變化圖表
            if len(intersection_data) > 0:
                plt.figure(figsize=(12, 8))
                steps_data = [data['step'] for data in intersection_data]
                robot1_coverage = [data['robot1_coverage'] for data in intersection_data]
                robot2_coverage = [data['robot2_coverage'] for data in intersection_data]
                intersection_coverage = [data['intersection_coverage'] for data in intersection_data]
                union_coverage = [data['union_coverage'] for data in intersection_data]
                
                # 繪製四條曲線
                plt.plot(steps_data, robot1_coverage, 'b-', linewidth=2, label='Robot 1')
                plt.plot(steps_data, robot2_coverage, 'r-', linewidth=2, label='Robot 2')
                plt.plot(steps_data, intersection_coverage, 'g-', linewidth=2, label='Intersection')
                plt.plot(steps_data, union_coverage, 'k-', linewidth=2, label='Union')
                
                # 添加標籤和標題
                plt.xlabel('Time (steps)', fontsize=14)
                plt.ylabel('Coverage', fontsize=14)
                plt.title(f'Time-Coverage Analysis - Hybrid Approach (Start Point {start_idx+1})', fontsize=16)
                
                # 添加網格和圖例
                plt.grid(True, linestyle='--', alpha=0.7)
                plt.legend(fontsize=12)
                
                # 設置y軸範圍
                plt.ylim(0, 1.05)
                
                plt.savefig(os.path.join(current_output_dir, 'time_coverage_analysis.png'), dpi=300)
                plt.close()
                
                print(f"已生成起始點 {start_idx+1} 的時間-覆蓋率分析圖表")
            
            # 停止追蹤
            tracker.stop_tracking()
            
            # 清理資源
            tracker.cleanup()
            
            # 清理機器人資源
            for robot in [robot1, robot2]:
                if robot is not None and hasattr(robot, 'cleanup_visualization'):
                    robot.cleanup_visualization()
            
            print(f"完成起始點 {start_idx+1} 的測試，總步數: {steps}")
            
        except Exception as e:
            print(f"測試過程中出錯: {str(e)}")
            import traceback
            traceback.print_exc()
            
            # 確保清理資源
            try:
                if tracker is not None:
                    tracker.cleanup()
                
                for robot in [robot1, robot2]:
                    if robot is not None and hasattr(robot, 'cleanup_visualization'):
                        robot.cleanup_visualization()
            except:
                pass
    
    # 生成所有起始點的比較圖表
    try:
        # 從CSV檔案讀取所有數據
        all_data = {}
        with open(csv_path, 'r') as csvfile:
            reader = csv.DictReader(csvfile)
            for row in reader:
                start_point = int(row['StartPoint'])
                step = int(row['Step'])
                
                if start_point not in all_data:
                    all_data[start_point] = {}
                    all_data[start_point]['steps'] = []
                    all_data[start_point]['robot1'] = []
                    all_data[start_point]['robot2'] = []
                    all_data[start_point]['intersection'] = []
                    all_data[start_point]['union'] = []
                
                all_data[start_point]['steps'].append(step)
                all_data[start_point]['robot1'].append(float(row['Robot1Coverage']))
                all_data[start_point]['robot2'].append(float(row['Robot2Coverage']))
                all_data[start_point]['intersection'].append(float(row['IntersectionCoverage']))
                all_data[start_point]['union'].append(float(row['UnionCoverage']))
        
        # 為每個起始點創建單獨的全面覆蓋率圖表
        for start_point in sorted(all_data.keys()):
            plt.figure(figsize=(12, 8))
            
            plt.plot(all_data[start_point]['steps'], all_data[start_point]['robot1'], 
                    'b-', linewidth=2, label='Robot 1')
            plt.plot(all_data[start_point]['steps'], all_data[start_point]['robot2'], 
                    'r-', linewidth=2, label='Robot 2')
            plt.plot(all_data[start_point]['steps'], all_data[start_point]['intersection'], 
                    'g-', linewidth=2, label='Intersection')
            plt.plot(all_data[start_point]['steps'], all_data[start_point]['union'], 
                    'k-', linewidth=2, label='Union')
            
            plt.xlabel('Time (steps)', fontsize=14)
            plt.ylabel('Coverage', fontsize=14)
            plt.title(f'Time-Coverage Analysis - Hybrid Approach (Start Point {start_point})', fontsize=16)
            plt.grid(True, linestyle='--', alpha=0.7)
            plt.legend(fontsize=12)
            plt.ylim(0, 1.05)
            
            plt.savefig(os.path.join(output_dir, f'time_coverage_startpoint_{start_point}.png'), dpi=300)
            plt.close()
        
        # 創建聯集覆蓋率比較圖表
        plt.figure(figsize=(12, 8))
        
        for start_point in sorted(all_data.keys()):
            plt.plot(all_data[start_point]['steps'], all_data[start_point]['union'], 
                    linewidth=2, label=f'Start Point {start_point}')
        
        plt.xlabel('Time (steps)', fontsize=14)
        plt.ylabel('Total Coverage (Union)', fontsize=14)
        plt.title('Total Coverage Comparison - Hybrid Approach', fontsize=16)
        plt.grid(True, linestyle='--', alpha=0.7)
        plt.legend(fontsize=12)
        plt.ylim(0, 1.05)
        
        plt.savefig(os.path.join(output_dir, 'all_total_coverage_comparison.png'), dpi=300)
        plt.close()
        
        print(f"已生成所有起始點的覆蓋率分析圖表")
        
    except Exception as e:
        print(f"生成比較圖表時出錯: {str(e)}")
    
    print(f"\n===== 完成所有起始點的測試 =====")
    print(f"結果儲存在: {output_dir}")
    print(f"覆蓋率數據儲存在: {csv_path}")
    print(f"測試完成！共生成了 {len(start_points_list)} 組不同起始點的時間-覆蓋率分析圖表")

def main():
    parser = argparse.ArgumentParser(description='測試多機器人混合探索策略')
    parser.add_argument('--map', type=str, default=os.path.join(os.getcwd(), 'data', 'DungeonMaps', 'test', 'img_6112b.png'),
                        help='地圖檔案路徑')
    parser.add_argument('--model', type=str, default=os.path.join(MODEL_DIR, 'best0111.h5'),
                        help='訓練好的模型路徑')
    parser.add_argument('--output', type=str, default='hybrid_results',
                        help='輸出目錄')
    
    args = parser.parse_args()
    
    # 檢查地圖檔案是否存在
    if not os.path.exists(args.map):
        print(f"警告: 在 {args.map} 找不到指定的地圖檔案")
        print("請提供正確的地圖檔案路徑。")
        exit(1)
    
    # 定義起始點位置 [robot1_pos, robot2_pos]
    start_points = [
        [[100, 100], [100, 100]],  # 起始點 1
        [[520, 120], [520, 120]],  # 起始點 2
        [[630, 150], [630, 150]],   # 起始點 3
        [[250, 130], [250, 130]],   # 起始點 4
        [[250, 100], [250, 100]],  # 起始點 5
        [[400, 120], [400, 120]],  # 起始點 6
        [[140, 410], [140, 410]],   # 起始點 7
        [[110, 590], [110, 590]],   # 起始點 8
        [[900, 300], [90, 300]],   # 起始點 9
        [[260, 200], [260, 200]],  # 起始點 10
    ]
    
    # 執行混合策略測試
    print("\n===== 執行混合策略測試 =====")
    test_hybrid_approach(args.map, start_points, args.model, output_dir=args.output)
    
    print("測試完成！")

if __name__ == '__main__':
    main()