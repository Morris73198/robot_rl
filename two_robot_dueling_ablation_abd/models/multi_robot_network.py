...
class MultiRobotNetworkModel:
    ...
    def _build_simple_frontier_module(self, frontier_input, robot_state):
        """構建簡單的frontier處理模塊 (去除前沿注意力C)"""
        # 簡單地處理frontier輸入
        ...
        # 與機器人狀態連接
        combined = layers.Concatenate()([x, robot_state])
    ...
    def _build_model(self):
        ...
        # 4. Frontier處理 (去除C)
        robot1_frontier = self._build_simple_frontier_module(frontier_input, robot1_coord)
        robot2_frontier = self._build_simple_frontier_module(frontier_input, robot2_coord)
